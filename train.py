...
from afi_sv import afi_sv_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = afi_sv_score(
            ...
            n_samples=config.N_FIDUCIAL_SAMPLES
        )
    ...
    top_etfs = [{"ticker": t, "afi_sv_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/afi_sv_{timestamp}.json"
