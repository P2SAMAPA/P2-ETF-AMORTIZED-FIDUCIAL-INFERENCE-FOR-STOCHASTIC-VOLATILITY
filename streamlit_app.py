...
st.set_page_config(page_title="Amortized Fiducial Inference for SV", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 AFI-SV: Amortized Fiducial Inference for SV</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Signature-based amortized inference | Fiducial distributions over H, nu, rho | Intraday recalibration</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 AFI-SV")
...
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **Samples:** {config.N_FIDUCIAL_SAMPLES}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'afi_sv_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>AFI-SV score: {etf['afi_sv_score_norm']:.3f}</p>
...
with tab1:
    st.header("🧠 Top ETFs by AFI-SV (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **AFI-SV** synthesizes Fiducial Inference, Amortized Variational RL, and GP-Vol.
        - Train an amortized inference network that maps price paths to fiducial distributions.
        - Parameters: **H** (Hurst exponent – roughness), **ν** (vol-of-vol), **ρ** (leverage).
        - Score = H (higher H = more persistent = more momentum).
        - Instant inference – no MCMC, feasible for intraday recalibration.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the AFI-SV scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("AFI-SV | Amortized Fiducial Inference for Stochastic Volatility")
